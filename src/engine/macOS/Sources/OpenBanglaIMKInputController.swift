...
class OpenBanglaIMKInputController: IMKInputController {
    ...
    private func writeMarkToClient(_ client: (IMKTextInput & IMKUnicodeTextInput),_ string: String) {
        openbanglaLog(logLevel: .VERBOSE, "writeMarkToClient - '\(string)'")
    ...
    private func writeTextToClient(_ client: (IMKTextInput & IMKUnicodeTextInput),_ string: String) {
        openbanglaLog(logLevel: .VERBOSE, "writeTextToClient = '\(string)'")
    ...
    private func handleKeyDown(_ event: NSEvent, _ sender: (IMKTextInput & IMKUnicodeTextInput)) -> Bool {
        ...
        openbanglaLog(logLevel: .VERBOSE, "Keypress event - keycode: \(keyCode) - alt: \(altgr) shift: \(shift)")
        ...
        openbanglaLog(logLevel: .VERBOSE, "Keypress event - keychar: '\(char)'")
        ...
        if let code = charToKeycode(char, macKeycode: keyCode) {
            keycode = code
        } else if altgr {
            if let code = macKeycodeToKeycode(keyCode, shift: shift) {
                keycode = code
            } else {
                openbanglaLog(logLevel: .ALWAYS_PRINT, "Unexpected AltGr mode keycode, rejecting keypress")
                return false
            }
        } else {
            openbanglaLog(logLevel: .ALWAYS_PRINT, "Unexpected keycode, rejecting keypress")
            return false
        }
